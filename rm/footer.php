                        ...
                        <p class="modal-alert"> O título será inserido na capa e cabeçalho dos relatórios. </p>
                            ...
                            <fieldset class="form-group">
                                <div class="form-group col-xs-12">
                                    <input type="text" id="pesquisaNome" name="pesquisaNome" class="form-control" placeholder="Dê um titulo para a pesquisa" required />
                                </div>
                                <input type="hidden" id="form" name="form" value="novo" >                            
                            </fieldset>
                    </div>
    ...
    <script src="<?php echo base_url()?>assets/dashboard/js/trumbowyg/dist/trumbowyg.min.js"></script>
    <script src="<?php echo base_url()?>assets/dashboard/js/autoSave.min.js"></script>
...
/////////////////////////////////////////////////////////////////////////////////////////// QUESTOES
    ...
    <?php if(!empty($pg_mapa)): ?>
    <script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBvLVxUfEauc2SsXlqDaT3dBkqaVXqwBLk"></script>
    <script type="text/javascript">
        var geocoder; //To use later
        var map; //Your map

        app.loading('.mapa','open')

        function closePanel(){

            $('.coleta-panel').addClass('slideOutLeft')

        }

        function getLocations(){
            $.get(ajaxUrl+'getColetasMapa/'+<?php echo $pesquisa->pesquisaID; ?>, function(data){

                //console.log(JSON.parse(data) )

                displayMarkers( JSON.parse(data) )
        ...
        function displayMarkers(data){
            ...
            for (var i = data.length - 1; i >= 0; i--) {
                ...
                var item = data[i].rastreamento.split(',')
                var latlng = new google.maps.LatLng(item[0],item[1]);

                createMarkers(latlng,data[i])

                bounds.extend(latlng); 
            }
 
            map.fitBounds(bounds);
            app.loading('.mapa','close')

        }

        function painelPesquisa(coletaID){

            $('.coleta-panel').addClass('slideOutLeft')


            $.get(ajaxUrl+'getColeta/'+coletaID, function(data){
                $('#dadoscoleta').html('')
                //console.log(JSON.parse(data) )
                var coletas = JSON.parse(data)
                for(var i = coletas.length -1; i >= 0; i--){

                    $('#dadoscoleta').prepend('<div class="questao"><h5>'+coletas[i].questaoID+' - '+coletas[i].questaoEnunciado+'</h5></div><div class="questao"><p>'+coletas[i].resposta+'</p></div>')
                }

                $('.coleta-panel').removeClass('slideOutLeft') 
                $('.coleta-panel').addClass('slideInLeft').show()
        ...
        function createMarkers(latlng,pesquisa){
            ...
            google.maps.event.addListener(marker, 'click', function() {
                ...
                var iwContent = '<div id="card-info-map animated fadeInUp" class="card-info-map p-1">'+
                    '<div class="row m-1 align-items-center">'+
                        '<div class="col-7">'+
                            '<div class="row px-3 align-items-center">'+
                                '<div class="col-12 px-0 py-3 card-info-map-dispositivo">'+
                                    '<div class="card-info-map-device-name">'+
                                        '<div class="marcamodelo"> Coletor '+pesquisa.coletorID+'</div>'+
                                    '</div>'+
                                    '<div class="card-info-map-device-ID" style="padding:8px 0"> Inicio: '+ msToTime(pesquisa.timeInicio) +'<br/><br/> Fim: '+  msToTime(pesquisa.timeFim).toLocaleString() +'</div></div>'+
                                '<div class="col-12 px-0 py-3 bd-top card-info-map-local">'+pesquisa.rastreamento+'</div>'+
                            '</div>'+
                        '</div>'+
                        '<div class="col-5 bd-left">'+
                            '<div class="row px-3 align-items-center text-center">'+
                                '<div class="col-12 px-0 card-info-map-veloc" style="padding:40px 0">'+ ( pesquisa.timeFim / 1000) +'</div>'+
                ...
                painelPesquisa(pesquisa.coletaID)
        ...
        function initialize() {
            ...
            //Default setup
            var latlng = new google.maps.LatLng(-16.592517, -49.369386);
            var myOptions = {
                zoom: 9,
                center: latlng,
                //scrollwheel: false,
        ...
        // var marker = new google.maps.Marker({
        //         position: {lat: -16.592517, lng: -49.369386}
        //     });
        ...
        //             });
        //         } else {
        //             $('#resultado').text("O endereço pode estar errado: " + status);
        //         }
        //     });
        // }
